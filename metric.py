...
if __name__ == "__main__":
    # 1. 使用 trimesh 加载网格
    pred_trimesh = trimesh.load("baoutput.obj")
    gt_trimesh = trimesh.load("bad_doll.obj")

    # 2. 确定设备 (假设您想使用 CUDA)
    DEVICE = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    
    # 3. 将 trimesh 对象转换为 PyTorch3D Meshes 对象
    #    - 转换为 torch.Tensor
    #    - 移动到正确的 DEVICE
    #    - 确保数据类型为 float32
    
    # 预测网格 (pred_meshes)
    pred_verts = torch.tensor(pred_trimesh.vertices, dtype=torch.float32, device=DEVICE)
    pred_faces = torch.tensor(pred_trimesh.faces, dtype=torch.int64, device=DEVICE)
    pred_meshes = Meshes(verts=[pred_verts], faces=[pred_faces])
    
    # 地面真实网格 (gt_meshes)
    gt_verts = torch.tensor(gt_trimesh.vertices, dtype=torch.float32, device=DEVICE)
    gt_faces = torch.tensor(gt_trimesh.faces, dtype=torch.int64, device=DEVICE)
    gt_meshes = Meshes(verts=[gt_verts], faces=[gt_faces])
    
    # 4. 调用 compare_meshes 函数
    print(compare_meshes(pred_meshes, gt_meshes))